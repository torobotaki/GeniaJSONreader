...
def getTypes(alldata):
    ...
    for data in alldata:
        ...
        sections[datasection.name] = datasection;
        for etypename in datasection.entityTypes:
            enttype = datasection.entityTypes[etypename]
            if etypename not in entityTypes: entityTypes[enttype] = 1;
            else: entityTypes[enttype] += 1
    ...
    return entityTypes, relationTypes, sections
...
def parseJSONdata(data):
    ...
    if "catanns" in data:
        for dataentity in data["catanns"]:
            ...
            if etype not in newsection.entityTypes: newsection.entityTypes[etype] = etype
    if 'insanns' in data:
...
entityTypes, relationTypes, section = getTypes(data)
pprint(entityTypes)
